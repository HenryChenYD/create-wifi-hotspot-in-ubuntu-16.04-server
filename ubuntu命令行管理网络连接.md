...
注意：
  ...
  2.使用‘nmcli device wifi connect’命令连接已知 WiFi 时，IP将是自动分配，为方便SSH连接，必须使用‘&&’搭配指定 IP 和重启网络连接的命令；
  3.如果不使用‘name LINK_NAME’来指定连接的名字，将会自动以对应的 SSID 来命名；
  4.不指定连接名地重复连接同一个 WiFi 时，从第二次开始会自动以 SSID_n 来命名，n 从1开始逐次加1，此时，在指定 IP 和重启网络连接的命令中，LINK_NAME 必须修改为对应的连接名。
